...
class User extends Controller
{
    ...
    public function authMessCodeOld()
    {
        ...
        if($res['id'] > 0 && $res['add_time'] > $time_minus10){
            ...
            if($user_info['id'] > 0){
                ...
                $user_id = $tmpuser_id =Db::table("think_customer")->insertGetId(array("mobile" => $mobile,"username"=>$mobile,"add_date"=>date("Y-m-d H:i:s")));
                if(strpos($user_id, '64')!==false||strpos($user_id, '89')!==false){
                    $user_id= str_replace('64', '65', $user_id);
                    $user_id= str_replace('89', '90', $user_id);
                    Db::table('think_customer')->where('id = ' . $tmpuser_id)->update(['id'=>$user_id]);
                }
    ...
    public function uploadsign()
    {
        ...
        if ($info) {
            ...
            $img = @file_get_contents($image_url);
            $file_name = rand(1111,9999) . time();
            $path = UPLOAD_PATH . 'authrealname/' . $file_name .'.png';
            $file_path = '/upload/authrealname/' . $file_name .'.png';
    ...
    public function getInsuranceInfo()
    {
        ...
        if($info){
            $user_info = Db::table("think_customer")->where(["id" => $customer_id])->find();
            $month = date("m",strtotime($user_info['add_date']));
            $info['user_code'] = get_user_code($month,$user_info['id']);
    ...
    public function authMessCode()
    {
        ...
        if($res['id'] > 0 && $res['add_time'] > $time_minus10){
            ...
            if($user_info['id'] > 0){
                ...
                $user_id =$tmpuser_id= Db::table("think_customer")->insertGetId(array("id" => $id,"mobile" => $mobile,"username"=>$mobile,"add_date"=>date("Y-m-d H:i:s"),"openid"=>$openid));
                if(strpos($user_id, '64')!==false||strpos($user_id, '89')!==false){
                    $user_id= str_replace('64', '65', $user_id);
                    $user_id= str_replace('89', '90', $user_id);
                    Db::table('think_customer')->where('id = ' . $tmpuser_id)->update(['id'=>$user_id]);
    ...
    // 身份证实名认证查询校验接口_身份证二要素API接口
    public function idenAuthentication(){
        $host = "https://idenauthen.market.alicloudapi.com";
        $path = "/idenAuthentication";
        $method = "POST";
        $appcode = "7b6ddfc9f49f4685adfd98f35defc483";
        $headers = array();
        array_push($headers, "Authorization:APPCODE " . $appcode);
        array_push($headers, "Content-Type".":"."application/x-www-form-urlencoded; charset=UTF-8");
        $querys = "";
        $bodys = "idNo=".input("post.idNo")."&name=".input("post.name")."";
        $url = $host . $path;
    ...
    //银行卡信息三元素（三要素）实名认证查询接口
    public function bankAuthenticate(){
        ...
        array_push($headers, "Content-Type".":"."application/x-www-form-urlencoded; charset=UTF-8");
        $querys = "";
        $bodys = "cardNo=".input("post.cardNo")."&idNo=".input("post.idNo")."&name=".input("post.name")."";
        $url = $host . $path;
    ...
    //     $res= Db::table("think_customer")->where(["id_number" => input("post.id_number"),"deleted"=>0])->find();
    //     if($res){
    //         $return = array(
    //             "code" => "400",
    //             "data" => "0",
    ...
    public function authRealName()
    {
        ...
        $id_card= Db::table("think_customer")->where(["id_number" => input("post.id_number"),"deleted"=>0])->find();
        if($id_card){
            $return = array(
                "code" => "400",
                "data" => "0",
        ...
        $res = db('customer')->where('id', $user_id)->update($data);
        if($res){
            $return = array(
                "code" => "200",
                "data" => "1",
    ...
    public function upload_imgs()
    {
        ...
        if($get['type']=='codenum'){//身份证
            ...
            $savepath =     '/imgs/face' ;
        } elseif($get['type']=='linshi'){//身份证临时上传
            $savepath =     '/imgs/linshi/'.$uid ;
        } else{
        ...
        if($info){
            ...
            }elseif($get['type']=='linshi'){//身份证临时
                if($get['imgIndex']=='b'){//背面
                    $updatecode['id_pic_b']=$src;
                    $res =$this->orcidcd($updatecode['id_pic_b'],$get['side']);
                    return $res;
                }
                if($get['imgIndex']=='a'){//正面
                    $updatecode['id_pic_a']=$src;
                    $res =$this->orcidcd($updatecode['id_pic_a'],$get['side']);
                    return $res;
                }
    ...
    public function orcidcd($updatecode,$side){
        $url = "https://dm-51.data.aliyun.com/rest/160601/ocr/ocr_idcard.json";
        $appcode = "7b6ddfc9f49f4685adfd98f35defc483";
        $file = 'C:/xampp/htdocs/public'.$updatecode; 
...
/**
    ...
    function   upfilehtml($id){
        ...
        if(is_object($file)){
            ...
            if($fileInfo['size'] > 1024*1024*$config['file_size']){
                // 上传失败获取错误信息
                echo  '<script type="text/javascript" >alert("文件超过' . $config['file_size'] . 'M" ); return  false;</script> '  ;
            ...
            if(!in_array($ext, $extArr)){
                echo  '<script type="text/javascript" >alert("只能上传' . $config['file_ext'] . '的文件" ); return  false;</script> '  ;
            }
            $savename=true;
        ...
        $html='<!DOCTYPE html>
                ...
                layui.use("upload", function(){
                    ...
                    upload.render({
                        elem: "#test8"
                        ,url: "https://www.51fengniao.cn/api/User/upfilehtml/id/"+'.$id.' //改成您自己的上传接口
                        ,auto: false
                        ,accept: "file" //普通文件
                        ,exts: "docx|pdf|doc" //只允许上传压缩文件
                        //,multiple: true
                        ,bindAction: "#test9"
            ...
            <body>
                ...
                <fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;text-align:center;">
                  ...
                  <div class="layui-upload">
                    <button type="button" class="layui-btn layui-btn-normal" id="test8" style="margin-left:10%">选择文件</button>
                    <button type="button" class="layui-btn" id="test9" style="margin-left:10%">开始上传</button>
                  </div>
